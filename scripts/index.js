...
const generateFormElement = function () {
    ...
    <form aria-label="Add a new bookmark" id="js-add-new"> 
      ...
      <input type="text" name="url" id="url" placeholder="https://www.google.com">
      ...
      <input type="text" name="desc" id="desc" placeholder="Description of website">
	...
	if (item.rating) {
		for (let i = 0; i< 5; i++) {
			if (i< parseInt(item.rating)) {
				...
				ratingDiv += `<span class="fa fa-star"></span>`
			}
		}
	} //ratingDiv = `<span class="fa fa-star"></span>`; // `<p class="rating">rating: ${item.rating} </p>`;
...
function render () {
	...
	const bookmarkItemsString = generateBookmarkDisplay(store.items.filter( item => getFilterRating() <= item.rating ));
